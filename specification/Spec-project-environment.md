...
- **FR-003**: The project MUST provide npm scripts for: starting the dev server against the mock API (`dev`, `dev:mock`) or against a placeholder Spring Boot target (`dev:api`), producing a production build (`build`), linting (`lint`), and previewing a production build (`preview`). `dev`/`dev:mock`/`dev:api` only choose which backend the Vite dev-server proxy targets (see FR-011) — there is still deliberately **no** npm script for the mock API server itself, which is started only via the Thinker VS Code extension's UI, since the underlying npm package was removed for a high-severity `npm audit` finding with no non-breaking fix (see `Spec-mock-api-server.md`'s Change Log).
...
- Adding the missing test runner or `test:api` script — tracked as a gap (FR-012), not implemented by this spec.
