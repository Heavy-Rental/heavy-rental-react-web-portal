...
2. Build requests against `{{baseUrl}}`, e.g.:
   ...
   The same pattern applies to `/api/users`, `/api/rentalPlans`, `/api/monthly-utilization`, and `/api/status-distribution`. Instant Quotation is the dedicated route config at `/api/recommendations/project-spec`, not a CRUD collection.
...
- 2026-08-09: Corrected the "Pointing the frontend at this mock server" appendix — `.env.api`'s `VITE_API_TARGET` changed from `http://localhost:8080` to `http://heavy-rental-rest-api:8080` (a container-network hostname for the Spring Boot backend, not a locally-run process). This mock server itself (`127.0.0.1:4010`, `dev`/`dev:mock`) is unaffected. See `Spec-project-environment.md` FR-011 and `Spec-frontend-api-integration.md` FR-002 for the matching updates.
