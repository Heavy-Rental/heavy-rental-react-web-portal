...
2. Build requests against `{{baseUrl}}`, e.g.:
   ...
   The same pattern applies to `/api/users`, `/api/rentalPlans`, `/api/monthly-utilization`, and `/api/status-distribution` (see §3 Resources & Endpoints above for the full route table).
