...
7. The onboarding "How can we help you today?" screen only offers two paths — "I Know What I Want" and "I Have Specs, Need a Recommendation" — with the "I'm Just Browsing" option removed from the UI (CHANGE-07). `OnboardingMode` in `src/app/types.ts` still includes an unused `"browse"` member; it is not offered in `ChooseModeScreen`.
...
**Change**: removed the "I'm Just Browsing" entry from `ChooseModeScreen.tsx`'s option list and its `onBrowse`/`Search`-icon plumbing; removed the matching `onBrowse={() => onDone("browse")}` call in `CustomerOnboarding.tsx`; simplified the now-two-way header ternary in `CustomerPortal.tsx` (`onboardingMode === "specs" ? ... : ...`, dropping the `"browse"` case); updated `specsPlan.test.ts`'s `shouldPromptDeliveryDetails` test to drop its `"browse"` assertion (renamed to "still prompts on Know / unset"). **Correction:** `OnboardingMode` in `app/types.ts` still includes `"browse"` as an unused leftover union member; it is not offered in the UI.
