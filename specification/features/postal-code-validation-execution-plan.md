...
| **Status** | **Phase 1** (Sub-tasks 1-5) — done, committed, merged with `develop`. **Phase 2** (Sub-tasks 6-8) — Sub-task 6 (spec updates) and Sub-task 7 (`api.ts` client) done; Sub-task 8 (`CustomerPortal.tsx` wiring) remaining. Note: Sub-task 7 was implemented ahead of Spring's routes actually going live (still `⏳` per `spring contract/rental-plan-site-address.md`) — the client code is ready, but won't work end-to-end against the real backend until Spring ships it. |
