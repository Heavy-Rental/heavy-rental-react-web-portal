...
- Q: The demo login accounts (`john@company.com`, `sarah@company.com`) don't match any seed user's email in `mock/db.json`, so no real `userId` could be resolved at login — how to fix? → A: Remap `ACCOUNTS` in `App.tsx` to the seed users' real emails (`alex.tan@example.sg` → customer, `ravi.kumar@example.sg` → admin) and resolve the matching numeric id via `userApi.list()` at login time.
...
- **FR-002**: All API requests MUST be routed through a Vite dev-server proxy (`/api` → `http://127.0.0.1:4010`, `vite.config.ts`) using relative paths — no absolute URLs, no environment variables, no CORS workarounds.
...
- **FR-010**: The admin pricing tab's recommendation-apply actions MUST persist updated `daily`/`weekly` rates to `/api/equipment` for every equipment item they affect.
- **FR-011**: Booking status values displayed and settable in the admin UI MUST use the mock API's vocabulary (`pending-deposit`, `deposit-paid`, `completed`, `cancelled`) exclusively — no separate UI-only status vocabulary.
...
- Assumes the demo login accounts in `App.tsx`'s `ACCOUNTS` map stay in sync with `mock/db.json`'s seed users by email; adding or renaming seed users requires updating `ACCOUNTS` to match.
- Assumes fields with no API equivalent (asset maintenance metadata, fleet deployment/lifecycle tracking, pricing floor/ceiling bounds, the day-level availability calendar) are acceptable as permanently client-local/synthetic rather than a gap to close later.
- Assumes the Vite dev-server proxy is a development-time-only mechanism; it has no equivalent in a `vite build` production bundle, since no live backend exists to point a production build at.
- Assumes booking mutations made through the UI are best-effort against the mock server's in-memory store — consistent with `Spec-mock-api-server.md`'s "no persistence across restarts" behavior, not a regression introduced here.
...
- Authentication and session security beyond the existing local demo-account login gate; the mock API itself has no auth (per `Spec-mock-api-server.md`).
- Automated tests exercising this wiring — no test runner exists in the project yet (`Spec-project-environment.md` FR-012 tracks this as a known gap).
- A production/non-dev API base URL configuration — this feature only wires the Vite dev server; there is still no live backend to point a deployed build at.
- Enforcing "one active rental plan per user" (`Spec-ui-heavy-machinery-portal.md` §4.5) at write time — the mock server accepts any rental-plan creation regardless of existing active plans; the frontend does not add client-side enforcement of this rule beyond what the seed data already models.
